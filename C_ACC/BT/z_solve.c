...
// of the sweep.
//---------------------------------------------------------------------
void z_lhsinit(int k, int j, int ni, double lhs[PROBLEM_SIZE+1][PROBLEM_SIZE+1][PROBLEM_SIZE+1][3][5][5])//(double lhs[][3][5][5], int ni)
{
  int i, m, n;
...
void z_matvec_sub(int k1, int j1, int i1, int first, int k2, int j2, int i2, int k3, int j3, int i3, double lhs[PROBLEM_SIZE+1][PROBLEM_SIZE+1][PROBLEM_SIZE+1][3][5][5], double rhs[KMAX][JMAXP+1][IMAXP+1][5])//(double ablock[5][5], double avec[5], double bvec[5])//x_matvec_sub(k, j, i, AA, i-1);//x_matvec_sub(lhs[k][j][i][first], rhs[k][j][second], rhs[k][j][i]);
{
  //---------------------------------------------------------------------
...
void z_matmul_sub(int k1, int j1, int i1, int first, int k2, int j2, int i2, int second, int k3, int j3, int i3, int third, double lhs[PROBLEM_SIZE+1][PROBLEM_SIZE+1][PROBLEM_SIZE+1][3][5][5])//(double ablock[5][5], double bblock[5][5], double cblock[5][5])
{
...
void z_binvcrhs(int k1, int j1, int i1, int first, int k2, int j2, int i2, int second, int k3, int j3, int i3, double lhs[PROBLEM_SIZE+1][PROBLEM_SIZE+1][PROBLEM_SIZE+1][3][5][5], double rhs[KMAX][JMAXP+1][IMAXP+1][5])//(double lhs[5][5], double c[5][5], double r[5]) //x_binvcrhs( lhs[k][j][i][BB], lhs[k][j][i][CC][, rhs[k][j][i][ );
{
  double pivot, coeff;
...
void z_binvrhs(int k1, int j1, int i1, int first, int k2, int j2, int i2, double lhs[PROBLEM_SIZE+1][PROBLEM_SIZE+1][PROBLEM_SIZE+1][3][5][5], double rhs[KMAX][JMAXP+1][IMAXP+1][5])//(double lhs[5][5], double r[5])
{
  double pivot, coeff;
...
void z_solve()
{
  ...
  for (j = 1; j <= grid_points[1]-2; j++) {
    ...
    for (i = 1; i <= grid_points[0]-2; i++) {
      ...
      z_lhsinit(j, i, ksize, lhs);//z_lhsinit(lhs[j][i], ksize);
      for (k = 1; k <= ksize-1; k++) {
        tmp1 = dt * tz1;
        tmp2 = dt * tz2;
      ...
      z_binvcrhs( j, i, 0, BB, j, i, 0, CC, 0, j, i, lhs, rhs );//z_binvcrhs( lhs[j][i][0][BB], lhs[j][i][0][CC], rhs[0][j][i] );
      ...
      for (k = 1; k <= ksize-1; k++) {
        ...
        z_matvec_sub(j, i, k, AA, k-1, j, i, k, j, i, lhs, rhs);//z_matvec_sub(lhs[j][i][k][AA], rhs[k-1][j][i], rhs[k][j][i]);
        ...
        z_matmul_sub(j, i, k, AA, j, i, k-1, CC, j, i, k, BB, lhs);//z_matmul_sub(lhs[j][i][k][AA], lhs[j][i][k-1][CC], lhs[j][i][k][BB]);
        ...
        //-------------------------------------------------------------------
        //#pragma acc routine (binvcrhs) worker
        z_binvcrhs( j, i, k, BB, j, i, k, CC, k, j, i, lhs, rhs );//z_binvcrhs( lhs[j][i][k][BB], lhs[j][i][k][CC], rhs[k][j][i] );
      }
      ...
      //---------------------------------------------------------------------
      //#pragma acc routine (matvec_sub) worker
      z_matvec_sub(j, i, ksize, AA, ksize-1, j, i, ksize, j, i, lhs, rhs);//z_matvec_sub(lhs[j][i][ksize][AA], rhs[ksize-1][j][i], rhs[ksize][j][i]);
      ...
      //---------------------------------------------------------------------
      //#pragma acc routine (matmul_sub) worker
      z_matmul_sub(j, i, ksize, AA, j, i, ksize-1, CC, j, i, ksize, BB, lhs);//z_matmul_sub(lhs[j][i][ksize][AA], lhs[j][i][ksize-1][CC], lhs[j][i][ksize][BB]);

      //---------------------------------------------------------------------
      // multiply rhs(ksize) by b_inverse(ksize) and copy to rhs
      //---------------------------------------------------------------------
      //#pragma acc routine (binvrhs) worker
      z_binvrhs( j, i, ksize, BB, ksize, j, i, lhs, rhs );//z_binvrhs( lhs[j][i][ksize][BB], rhs[ksize][j][i] );
